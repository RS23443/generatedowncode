...
public class Odometry {
    ...
    public Odometry(HardwareMap hardwareMap, double ticksPerInch, double wheelBaseWidth, double horizontalOffset) {
        leftEncoder = new OverflowEncoder(new RawEncoder(hardwareMap.get(DcMotorEx.class, "left_back_drive")));
        rightEncoder = new OverflowEncoder(new RawEncoder(hardwareMap.get(DcMotorEx.class, "right_front_drive")));
        horizontalEncoder = new OverflowEncoder(new RawEncoder(hardwareMap.get(DcMotorEx.class, "right_back_drive")));
    ...
    // Updated getPosition method to return encoder positions
    public int[] getPosition() {
        int left = leftEncoder.getPositionAndVelocity().position - leftEncoderStart;
        int right = rightEncoder.getPositionAndVelocity().position - rightEncoderStart;
        int horizontal = horizontalEncoder.getPositionAndVelocity().position - horizontalEncoderStart;

        return new int[]{left, right, horizontal};
    }
    public void updatesmiple() {
        // Basic odometry without adaptive quadrature
        int deltaLeft = leftEncoder.getPositionAndVelocity().position - leftEncoderStart;
        int deltaRight = rightEncoder.getPositionAndVelocity().position - rightEncoderStart;
        int deltaHorizontal = horizontalEncoder.getPositionAndVelocity().position - horizontalEncoderStart;

        double deltaHeading = (deltaRight - deltaLeft) / wheelBaseWidth;
        heading = Math.min(Math.toRadians(heading += deltaHeading), Math.toRadians(360));

        x += deltaHorizontal / ticksPerInch * Math.cos(heading);
        y += deltaHorizontal / ticksPerInch * Math.sin(heading);
    }
}
